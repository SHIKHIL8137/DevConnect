import { fetchClientData, fetchClientDatas, getClientsDatas } from "../api/userServiceApi.js";
import { fetchVerificationRequest, updateVerification, verificationRequestData } from "../api/verificationService.js";
...
export const getClientVerificationDetails = async(req,res)=>{
  try {
    const {id} = req.query;
    if(!id) return res.status(404).json({status:false,message:"the id is missing!!"});
...
export const verifyOrRejectUser =async(req,res)=>{
  try {
    ...
    if(!updateVerifiyClient.data.status) return res.status(400).json({status:false,message:"error occures"})
      res.status(200).json({status:true,data:updateVerifiyClient.data,message:updateVerifiyClient.data.message})
  } catch (error) {
     console.error(error);
    return res.status(500).json({
      success: false,
