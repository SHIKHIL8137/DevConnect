...
export const getVerificationData = async(req,res)=>{
  try {
    ...
    if(!requests) return res.status(400).json({status:false,message:"error occuers"});
...
export const updateVerification = async (req, res) => {
  try {
    ...
    if (!request) {
      return res.status(404).json({ status: false, message: "Verification request not found" });
    }

    if (request.clientId !== userId) {
      return res.status(403).json({ status: false, message: "Request does not belong to this user" });
    }
