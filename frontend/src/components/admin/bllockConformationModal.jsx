...
const ConfirmBlockModal = ({ isOpen, onClose, onConfirm, user, actionType }) => {
  if (!isOpen) return null;

  const isClient = user?.companyName !== undefined;
  const userType = isClient ? 'Client' : 'Freelancer';
  
  const title = actionType === 'block' ? `Block ${userType}` : `Unblock ${userType}`;
  ...
  const confirmButtonClass = actionType === 'block' 
        ...
        <div className="p-6">
          <div className="flex items-center mb-4">
            {user?.profileImage ? (
              ...
              <div className={`w-12 h-12 rounded-lg flex items-center justify-center mr-3 text-white ${
                isClient 
                  ? "bg-gradient-to-br from-blue-400 to-indigo-500" 
                  : "bg-gradient-to-br from-purple-400 to-indigo-500"
              }`}>
                <span className="font-medium text-lg">{user?.userName?.charAt(0)}</span>
              </div>
            )}
            <div>
              <p className="font-medium">{user?.userName}</p>
              <p className="text-sm text-gray-500">{isClient ? (user?.companyName || "Individual") : user?.position}</p>
            </div>
          </div>
