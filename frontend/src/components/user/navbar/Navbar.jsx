    ...
    <header className="w-full py-4 px-8 bg-white shadow-sm md:rounded-full mx-auto max-w-7xl">
      <div className="flex items-center justify-between">
        <div className="flex items-center" onClick={()=>navigate('/')}>
          <span className="text-blue-500 text-2xl font-bold">&lt; &gt; Dev</span>
          <span className="text-gray-400 text-2xl font-bold">Connect</span>
        </div>
        <nav className="hidden md:flex items-center space-x-8">
      <p
        onClick={() => navigate('/')}
        className={`cursor-pointer ${path === '/' ? 'text-blue-500' : 'text-gray-600'} hover:text-blue-500`}
      >
        Home
      </p>
      ...
      {user?.role === 'client' ? (
        <p
          ...
          className={`cursor-pointer ${path === '/client/home' ? 'text-blue-500' : 'text-gray-600'} hover:text-blue-500`}
        >
          Find Freelancers
        </p>
        ...
        <p
          ...
          className={`cursor-pointer ${path === '/freelancer/home' ? 'text-blue-500' : 'text-gray-600'} hover:text-blue-500`}
        >
          Find Project
        </p>
      ) : (
      ...
      <p
        ...
        className={`cursor-pointer ${path === '/about' ? 'text-blue-500' : 'text-gray-600'} hover:text-blue-500`}
      >
        About Us
      </p>
      <p
        onClick={() => navigate('/contact')}
        className={`cursor-pointer ${path === '/contact' ? 'text-blue-500' : 'text-gray-600'} hover:text-blue-500`}
      >
        Contact Us
      </p>
    </nav>
      ...
      {isMenuOpen && (
        <div className="md:hidden mt-4 pb-4">
          <nav className="flex flex-col space-y-4">
            ...
            {user?.role === 'client' ? (
              <a href="#" className="text-gray-600 hover:text-blue-500">Find Freelancers</a>
            ) : user?.role === 'freelancer' ? (
              <a href="#" className="text-gray-600 hover:text-blue-500">Find Project</a>
            ) : (
              <>
                <a href="#" className="text-gray-600 hover:text-blue-500 mr-4">Find Freelancers</a>
                <a href="#" className="text-gray-600 hover:text-blue-500">Find Project</a>
              </>
            )}
